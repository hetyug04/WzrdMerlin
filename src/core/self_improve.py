"""
WzrdMerlin v3 — ImprovementManager

v3 changes: ModelInterface → OllamaClient in _apply_patch().
...
class ImprovementManager(BaseActor):
    ...
    Listens for CAPABILITY_GAP events. When an agent flags a missing capability:
      ...
      2. Uses OllamaClient to generate a real code patch
    ...
    async def handle_improvement_queued(self, event: Event):
        ...
        try:
            ...
            if not is_valid:
                logger.warning(f"Candidate failed validation — retrying with error context")
                retry_ok = await self._apply_patch(
                    candidate_dir, gap_desc, task_src, error_context=validation_log
                )
                if retry_ok:
                    is_valid, validation_log = self._validate_candidate(candidate_dir)
    ...
    async def _apply_patch(
        ...
        """Use OllamaClient to generate an actual code patch for the capability gap."""
        ...
        # Inject relevant atomic facts from gardener memory
        memory_context = ""
        try:
            mem = get_memory()
            facts = await mem.recall(gap_desc, top_k=3)
            if facts:
                memory_context = f"\nRELEVANT MEMORY FROM PREVIOUS ATTEMPTS:\n{facts}\n"
        except Exception:
            pass

        # Inject validation error from previous attempt if retrying
        error_section = ""
        if error_context:
            error_section = f"\nPREVIOUS ATTEMPT FAILED WITH:\n{error_context[:800]}\nFix the above error in this attempt.\n"
...
"""
        ...
        try:
            raw = await ollama.chat(
                messages=[
                    {"role": "system", "content": "You are a code generation assistant. Output only valid JSON."},
                    {"role": "user", "content": patch_prompt},
                ]
            )
            if not raw:
                logger.error("OllamaClient returned empty response for patch")
                return False
            ...
            subprocess.run(["git", "add", "."], cwd=candidate_dir, check=True, capture_output=True)
    ...
    def _validate_candidate(self, candidate_dir: str) -> Tuple[bool, str]:
        ...
        if os.path.isdir(tests_dir):
            pytest_res = subprocess.run(
                ...
                cwd=candidate_dir, capture_output=True, text=True, timeout=60,
            )
        ...
        import_check = subprocess.run(
            ...
            cwd=candidate_dir, capture_output=True, text=True, timeout=15,
        )
    ...
    def _promote_candidate(self, candidate_dir: str, branch_name: str):
        try:
            subprocess.run(["git", "add", "."], cwd=candidate_dir, capture_output=True)
            subprocess.run(
    ...
    def _cleanup_worktree(self, candidate_dir: str, branch_name: str):
        ...
        try:
            subprocess.run(
                ["git", "worktree", "remove", "-f", candidate_dir], capture_output=True
            )
    ...
    async def rollback_last(self) -> str:
        ...
        try:
            ...
            with open(ROLLBACK_LOG, "w") as f:
                json.dump(rollbacks, f, indent=2)
            return f"Rolled back to {target_hash[:8]} (reverted branch {branch})."
        except subprocess.CalledProcessError as e:
